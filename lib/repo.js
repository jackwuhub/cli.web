...
const configUrl = 'https://github.com/jackwuhub/cli.web/releases/download/config/repoTree.json'
const giteeConfigUrl = 'https://gitee.com/wuguangshuai/cli.web/releases/download/config/repoTree.json'
...
const getRepoConfig = async (netWork = false) => {
    if (!netWork) return Promise.resolve(localConfig)
        ...
        let config = await fetch(configUrl, {timeout: 10000}).
        then(res => res.json()).
        catch(async () => {
            spinner.fail('GitHub仓库获取失败, 切换为Gtiee仓库...')
            return await fetch(giteeConfigUrl, {timeout: 10000}).
            then(res => res.json()).
            catch(async () => {
                spinner.fail('加载仓库失败...')
        ...
        config ? spinner.succeed('加载仓库成功...') : spinner.fail('加载仓库失败...')
