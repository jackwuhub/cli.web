...
const configUrl = 'https://github.com/jackwuhub/cli.web/releases/download/untagged-be9c1c78e39624859a71/repoTree.json'
...
const getRepoConfig = async (netWork = false) => {
    if(!netWork) return Promise.resolve(localConfig)
        ...
        let config = await fetch(configUrl).then(res => {
            console.log(res)
        })
